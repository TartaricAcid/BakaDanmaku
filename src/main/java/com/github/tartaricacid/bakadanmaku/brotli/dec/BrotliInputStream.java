...
import com.github.tartaricacid.bakadanmaku.brotli.dec.BrotliRuntimeException;
...
public class BrotliInputStream extends InputStream {
   ...
   * Decoder state.
   */
  private final com.github.tartaricacid.bakadanmaku.brotli.dec.State state = new com.github.tartaricacid.bakadanmaku.brotli.dec.State();
  ...
  public BrotliInputStream(InputStream source, int byteReadBufferSize,
    ...
    this.bufferOffset = 0;
    try {
      com.github.tartaricacid.bakadanmaku.brotli.dec.State.setInput(state, source);
    } catch (BrotliRuntimeException ex) {
      throw new IOException("Brotli decoder initialization failed", ex);
    }
    if (customDictionary != null) {
      com.github.tartaricacid.bakadanmaku.brotli.dec.Decode.setCustomDictionary(state, customDictionary);
    }
  }
  ...
  public void close() throws IOException {
    com.github.tartaricacid.bakadanmaku.brotli.dec.State.close(state);
  }
  ...
  public int read(byte[] destBuffer, int destOffset, int destLen) throws IOException {
    ...
    try {
      ...
      state.outputUsed = 0;
      com.github.tartaricacid.bakadanmaku.brotli.dec.Decode.decompress(state);
