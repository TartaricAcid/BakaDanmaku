...
public class BilibiliSite implements ISite {
    ...
    public void initMessage(WebSocketClient client) {
        ...
        if (message == null) {
            NeoForge.EVENT_BUS.post(new SendDanmakuEvent("房间获取失败！请检查是否输入错误，或者网络有问题"));
            return;
        } else {
            NeoForge.EVENT_BUS.post(new SendDanmakuEvent("房间获取成功！正在连接弹幕！"));
        }
    ...
    private void handBufferMessage(ByteBuf data) throws Exception {
        ...
        if (operation == POPULAR_OPERATION) {
            NeoForge.EVENT_BUS.post(new UpdatePopularInfoEvent(data.getInt(BODY_OFFSET)));
            return;
        }
    ...
    private void handStringMessage(String message) {
        try {
            ...
            if (str != null) {
                NeoForge.EVENT_BUS.post(new SendDanmakuEvent(str));
            }
